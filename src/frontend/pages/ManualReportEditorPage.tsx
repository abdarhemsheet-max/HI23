...
export default function ManualReportEditorPage() {
  ...
  const [savedId, setSavedId] = useState<string | null>(isNew ? null : params.id ?? null);
  ...
  const save = async (): Promise<string | null> => {
    ...
    if (result) {
      if (!savedId) {
        ...
        navigate(`/reports/manual/${result.id}`, { replace: true });
      }
      return result.id;
    }
    return null;
  ...
  if (notFound) {
    return (
      <div className="py-20 text-center">
        ...
        <button className="btn-ghost mx-auto mt-4" onClick={() => navigate('/reports')}>
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button className="btn-ghost !px-3 !py-2 text-xs" onClick={() => navigate('/reports')}>
          <ArrowRight size={14} /> رجوع للتقارير
        </button>
