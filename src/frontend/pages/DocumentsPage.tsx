...
export default function DocumentsPage() {
  ...
  const delDoc = async (d: Document) => {
    const ok = await confirm({
      title: 'تأكيد الحذف',
      description: `سيُحذف الملف «${d.name}» نهائياً من الأرشيف ومن B2 ولا يمكن استرجاعه.`,
      danger: true,
    });
    if (!ok) return;
    const res = await api(`/api/documents/${d.id}`, { method: 'DELETE' });
    if (res) {
      if (viewer?.id === d.id) closeViewer();
      load();
    }
  };
  ...
  /** رابط B2 مؤقت (7 أيام) — يُجلب عند الحاجة بدلاً من مسار API ثابت */
  const openViewer = async (d: Document) => {
    setViewer(d);
    setViewerUrl(null);
    try {
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">مستنداتك مخزَّنة على Backblaze B2 — تُقرأ داخل النظام مباشرة</p>
        </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-2.5 flex items-center gap-2">
                      {canPreview(d) ? (
                        <button className="btn-ghost !px-3 !py-1.5 text-[11px]" onClick={() => openViewer(d)}>
                          <Eye size={12} /> {isPdf(d) ? 'قراءة PDF' : 'عرض'}
                        </button>
                      ) : (
                        <button className="btn-ghost !px-3 !py-1.5 text-[11px]" onClick={() => downloadFile(d)}>
        ...
        <div className="print-hidden fixed inset-0 z-50 flex flex-col bg-black/80 backdrop-blur-md p-3 lg:p-6">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button className="btn-ghost !px-3 !py-1.5 text-[11px]" onClick={() => downloadFile(viewer)}>
                <Download size={12} /> تنزيل
              </button>
              <button className="btn-ghost !p-2" onClick={closeViewer} aria-label="إغلاق">
                <X size={16} />
              </button>
            </div>
          </div>
          <div className="glass flex-1 overflow-hidden !rounded-xl bg-white/95">
            {!viewerUrl ? (
              <div className="flex h-full items-center justify-center text-sm text-slate-500">جارٍ تجهيز الملف…</div>
            ) : isPdf(viewer) ? (
              <iframe src={`${viewerUrl}#toolbar=1`} title={viewer.name} className="h-full w-full border-0" />
            ) : (
