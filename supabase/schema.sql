...
create table "Wallet" (
  ...
  -- صاحب المال في محافظ الأمانات (مثل: الوالد) — يصير اسم الدين عند السلفة
  "ownerName" text,
...
create table "Transaction" (
  ...
  -- تصنيف مُقنّن للتحليل — 'transfer' يُستثنى دائماً من الدخل والمصروف
  "categoryKey" text not null default 'general' check ("categoryKey" in (
    'general',
  ...
  -- يربط طرفَي التحويل الداخلي في عملية واحدة تُحذف معاً
  "transferId" uuid,
  "createdAt" timestamptz not null default now()
);
create index "Transaction_walletId_idx" on "Transaction"("walletId");
create index "Transaction_transferId_idx" on "Transaction"("transferId");
create index "Transaction_categoryKey_idx" on "Transaction"("categoryKey");
create index "Transaction_expectedDate_idx" on "Transaction"("expectedDate");
...
create table "Debt" (
  ...
  -- الدين الناتج عن تحويل داخلي (سلفة) — يُحذف معه إن حُذف التحويل
  "transferId" uuid,
  "createdAt" timestamptz not null default now()
);
create index "Debt_transferId_idx" on "Debt"("transferId");
...
-- سقف إنفاق شهري لكل تصنيف — صف واحد لكل تصنيف لكل مستخدم
create table "Budget" (
  id uuid primary key default gen_random_uuid(),
  "userId" uuid not null default auth.uid() references auth.users(id) on delete cascade,
...
create policy "owner_all" on "Budget" for all using (auth.uid() = "userId") with check (auth.uid() = "userId");
...
-- ------------------------------------------------------------------
-- 8) معرض الأعمال (Portfolio)
--    أرشيف روابط ما أنجزته: فيديو، منشور، تصميم/صورة، أو موقع.
--    كتلة مستقلة (جدول + RLS معاً) لأنها أُضيفت بعد المخطط الأساسي —
--    نسخة مطابقة لـ migrations/20260731120000_add_works.sql.
-- ------------------------------------------------------------------
